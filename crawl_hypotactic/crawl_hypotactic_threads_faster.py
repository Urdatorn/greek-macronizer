...
NB: This faster version stops as soon as it finds a single instance of a word, and trusts that instance, instead of collating all instances.

NB2: Dichrona in diphthongs need to not be searched for!!!

use is_diphthong from filter dichrona with the logic:
...
def process_html_file(file_path, input_word):
    # This function processes a single HTML file
    with open(file_path, 'r', encoding='utf-8') as file:
        html_content = file.read()
    return find_syllable_lengths(html_content, input_word)
...
def process_html_files_in_folder(input_word):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # Prepare future tasks for processing HTML files
        future_to_file = {executor.submit(process_html_file, file_path, input_word): file_path for file_path in html_files}
        
        # Process the results as they are completed
        for future in tqdm(concurrent.futures.as_completed(future_to_file), total=len(html_files), desc="Processing files"):
